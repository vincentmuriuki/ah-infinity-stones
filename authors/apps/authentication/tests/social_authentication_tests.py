"""This module runs tests for user login process via Facebook, Twitter or Google"""
import os
import json
...
class UserTestCase(TestCase):
    """This class contains tests for the user login process through Twiiter, Facebook or Google"""
    def setUp(self):
    ...
    def test_google_login_no_access_key(self):
        """this method test that a user cannot login via Google if there's no access_key"""

        data = {"provider": "google",
                "access_key_secret":self.google_secret}
        response = self.client.post('api/socialAuth', data=data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(json.loads(response.data)["message"], "Google access_key is missing")
        self.assertEqual(json.loads(response.data)["token"], None)
    def test_google_login_no_access_key_secret(self):
        """this method test that a user cannot login via Google if there's no access_key_secret"""
    ...
    def test_google_login(self):
        ...
        self.assertEqual(json.loads(response.data)["message"], "User logged in successfully.")
        self.assertNotEqual(json.loads(response.data)["token"], None)
    def test_twitter_login_no_access_key(self):
        """this method test that a user cannot login via Twitter if there's no access_key"""

        data = {"provider": "twitter",
                "access_key_secret": self.twitter_secret}
        response = self.client.post('api/socialAuth', data=data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(json.loads(response.data)["message"], "Twitter access_key is missing")
        self.assertEqual(json.loads(response.data)["token"], None)
    def test_twitter_login_no_access_key_secret(self):
        """this method test that a user cannot login via Twitter if there's no access_key_secret"""
    ...
    def test_facebook_login_no_access_key(self):
        """this method test that a user cannot login via Facebook if there's no access_key"""

        data = {"provider": "facebook",
                "access_key_secret": self.facebook_secret}
        response = self.client.post('api/socialAuth', data=data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(json.loads(response.data)["message"], "Facebook access_key is missing")
        self.assertEqual(json.loads(response.data)["token"], None)
    def test_facebook_login_no_access_key_secret(self):
        """this method test that a user cannot login via Facebook if there's no access_key_secret"""
